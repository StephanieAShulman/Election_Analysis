# THIS IS THE UPDATED CODE EVEN IF GIT ADD DOESN'T RECOGNIZE IT.
#  IT I/O ERROR WAS CORRECTED BY INDENTING EVERYTHING BELOW THE IF STATEMENT.
...
with open(file_to_load) as election_data:
    ...
    for row in file_reader:
        ...
        # If the candidate does not match anyone existing, add the name to the list
        if candidate_name not in candidate_options:
            # Add the candidate name to the candidate list
            candidate_options.append(candidate_name)
            # And begin tracking the candidate's vote count
            candidate_votes[candidate_name] = 0
        # Add a vote to that candidate's count (needs to outside if statement to add up)
...
with open(file_to_save, "w") as txt_file:
    ...
    # Determine the winning vote count, percentage and candidate
